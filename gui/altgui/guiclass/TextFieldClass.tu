...
class TextFieldClass
	...
	BlinkCursor, SetActive, SetEchoChar, MoveToPrev,
    % The package interface
	MakeInactive, RemoveCursor, IsActive, MoveToNext, SetEnterProc
    ...
    var enterActionProc : procedure x (field : ^TextFieldClass)
    ...
    proc SetEnterProc (prc : procedure x (field : ^TextFieldClass))
	enterActionProc := prc
    end SetEnterProc
	...
	if echoChar = chr (0) then
		...
		y1 + fontDescent, textFontID, tColour)
	end if
	View.ClipOff
    end DrawText
	...
	if border = WidgetGlobals.LINE then
	    ...
	    height := theight + 4
	elsif border = WidgetGlobals.NOBORDER then
	    height := theight
	    width := originalWidth
	    x := originalX
	    y := originalY
    ...
    fcn MoveToPrev () : boolean
	% Check widgets
	var widget : ^GenericWidgetBodyClass := prevWidget
	loop
	    exit when widget = nil
	    if widget -> GetWindow = window and
		    objectclass (widget) >= TextFieldClass then
    ...
    fcn MoveToNext (wrap : boolean) : boolean
	% Check widgets
	...
	if enabled and showing and active then
		...
		SetText (text + "    ")
		SetSelection (9999, 9999)
	    elsif key = upArrow then
		result MoveToPrev ()
	    elsif key = downArrow then
		result MoveToNext (false)
	    elsif key = '\n' then
		result MoveToNext (false)
		%result false
	    end if
